...
////    private List<RoadDTO> dijkstra(List<RoadDTO> boundaryList) {
////
////        double[] dist = new double[boundaryList.size() + 1];
////        boolean[] visit = new boolean[boundaryList.size() + 1];
////        List<RoadDTO> saveRoute = new ArrayList<>();
////        double[][] maps = new double[boundaryList.size()][boundaryList.size()];
////        //각 노드별로 가중치 새기기
////
////        //배열 초기화
...
////        //출발지점 0
////        dist[0] = 0;
////        saveRoute.add(new RoadDTO(boundaryList.get(0).getLat(), boundaryList.get(0).getLon()));
////        double D = 0;
////        for(int i=0;i<boundaryList.size();i++){
////            for(int j=0;j<boundaryList.size();j++){
////                 D = distance(boundaryList.get(i).getLat(), boundaryList.get(i).getLon(), boundaryList.get(j).getLat(), boundaryList.get(j).getLon(), "K");
////                maps[i][j] = D;
////            }
////        }
////
...
////            for (int j = 0;j < boundaryList.size(); j++) {
////
////                //방문하지않았고 가장 가까운 거리와 가장 가까운 꼭지점을 구한다.
...
////
////            for (int j = 0; j < boundaryList.size(); j++) {
////                if (!visit[j] && maps[Node][j] != Integer.MAX_VALUE) {
////                    if (dist[Node] + maps[Node][j] < dist[j]) {
////                        dist[j] = (dist[Node] + maps[Node][j]);
////                        saveRoute.add(new RoadDTO(boundaryList.get(Node).getLat(), boundaryList.get(Node).getLon()));
////                    }
////
////                    //최단거리 갱신 경로
////                }
...
////        }
////
////        System.out.println("최단거리"+dist[boundaryList.size()-1]);
////        return saveRoute;
////    }
////
//
